...
public class PayAli extends BaseEntity{
    ...
    @ApiModelProperty(value = "公用回传参数，如果请求时传递了该参数，则返回给商户时会回传该参数。支付宝只会在同步返回（包括跳转回商户网站）和异步通知时将该参数原样返回。本参数必须进行UrlEncode之后才可以发送给支付宝。")
    ...
    @ApiModelProperty(value = "可用渠道,用户只能在指定渠道范围内支付，多个渠道以逗号分割。注，与disable_pay_channels互斥。渠道列表：https://docs.open.alipay.com/common/wifww7")
    private String enablePayChannels;

    @ApiModelProperty(value = "商户门店编号")
    private String storeId;

    @ApiModelProperty(value = "禁用渠道,用户不可用指定渠道支付，多个渠道以逗号分割。注，与enable_pay_channels互斥。渠道列表：https://docs.open.alipay.com/common/wifww7")
    private String disablePayChannels;

    @ApiModelProperty(value = "PC扫码支付的方式，支持前置模式和跳转模式。前置模式是将二维码前置到商户的订单确认页的模式。需要商户在自己的页面中以 iframe 方式请求。支付宝页面。具体分为以下几种：0：订单码-简约前置模式，对应 iframe 宽度不能小于1：订单码-前置模式，对应iframe 宽度不能小于 300px，高度不能小于600px；600px，高度不能小于300px；3：订单码-迷你前置模式，对应 iframe 宽度不能小于 75px，高度不能小于75px；4：订单码-可定义宽度的嵌入式二维码，商户可根据需要设定二维码的大小。跳转模式下，用户的扫码界面是由支付宝生成的，不在商户的域名下。2：订单码-跳转模式")
