...
//    @ApiImplicitParams({@ApiImplicitParam})
	...
	@GetMapping(value = "/list", produces = MediaType.APPLICATION_JSON_VALUE, consumes =
			MediaType.APPLICATION_JSON_VALUE)
	@ApiOperation(value = "批量查询成长值变化历史记录", notes = "批量查询成长值变化历史记录", responseContainer = "List", response =
			RespOk.class)
	public RespOk findAccountList(@RequestBody GrowthChangeHistory growthChangeHistory) {
		List<GrowthChangeHistory> growthChangeHistoryList =
				growthChangeHistoryService.list(new QueryWrapper<>(growthChangeHistory));
