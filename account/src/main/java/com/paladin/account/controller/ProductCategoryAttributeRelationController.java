...
//    @ApiImplicitParams({@ApiImplicitParam})
	...
	public RespOk findAccount(@RequestBody ProductCategoryAttributeRelation productCategoryAttributeRelation) {
		ProductCategoryAttributeRelation result =
				productCategoryAttributeRelationService.getById(productCategoryAttributeRelation);
		return new RespOk(200, "查询成功", result);
	}

	@GetMapping(value = "/list", produces = MediaType.APPLICATION_JSON_VALUE, consumes =
			MediaType.APPLICATION_JSON_VALUE)
	@ApiOperation(value = "批量查询产品分类和属性的关系", notes = "批量查询产品分类和属性的关系", responseContainer = "List", response =
			RespOk.class)
	public RespOk findAccountList(@RequestBody ProductCategoryAttributeRelation productCategoryAttributeRelation) {
		List<ProductCategoryAttributeRelation> productCategoryAttributeRelationList =
				productCategoryAttributeRelationService.list(new QueryWrapper<>(productCategoryAttributeRelation));
