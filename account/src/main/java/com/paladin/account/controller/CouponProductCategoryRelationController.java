...
//    @ApiImplicitParams({@ApiImplicitParam})
    ...
    public RespOk findAccount(@RequestBody CouponProductCategoryRelation couponProductCategoryRelation) {
        CouponProductCategoryRelation result = couponProductCategoryRelationService.getById(couponProductCategoryRelation);
        return new RespOk(200, "查询成功", result);
    }

    @GetMapping(value = "/list", produces = MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "批量查询优惠券产品分类关系", notes = "批量查询优惠券产品分类关系", responseContainer = "List", response = RespOk.class)
    public RespOk findAccountList(@RequestBody CouponProductCategoryRelation couponProductCategoryRelation) {
        List<CouponProductCategoryRelation> couponProductCategoryRelationList = couponProductCategoryRelationService.list(new QueryWrapper<>(couponProductCategoryRelation));
