...
//    @ApiImplicitParams({@ApiImplicitParam})
    @ApiOperation(value = "添加商品评价", notes = "添加商品评价", response = RespOk.class)
    public RespOk addCarItem(@RequestBody Comment comment) {
    ...
    public RespOk deleteAccount(@RequestBody Comment comment) {
    ...
    public RespOk modifyAccount(@RequestBody Comment comment) {
    ...
    public RespOk findAccount(@RequestBody Comment comment) {
        Comment result = commentService.getById(comment);
        return new RespOk(200, "查询成功", result);
    }

    @GetMapping(value = "/list", MediaType.APPLICATION_JSON_VALUE, consumes = MediaType.APPLICATION_JSON_VALUE)
    @ApiOperation(value = "批量查询商品评价", notes = "批量查询商品评价", responseContainer = "List", response = RespOk.class)
    public RespOk findAccountList(@RequestBody Comment comment) {
        List<Comment> commentList = commentService.list(new QueryWrapper<>(comment));
        return new RespOk(200, "查询成功", commentList);
    }
}
